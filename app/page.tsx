...
export default async function Dashboard() {
    ...
    return (
      <main className="mx-auto max-w-3xl p-8">
        ...
        <Card className="mt-6">
          ...
          <Text>
            Check that <code>GOOGLE_SHEET_ID</code>, <code>GOOGLE_SERVICE_ACCOUNT_EMAIL</code>, and{" "}
            <code>GOOGLE_PRIVATE_KEY</code> are set in your environment, and that the service
            account email has been shared on the Google Sheet.
          </Text>
  ...
  const monthProgressPct = Math.min(100, (m.totalEstimateLow / MONTHLY_REVENUE_TARGET) * 100)
    ...
    <>
      ...
      <main className="mx-auto max-w-7xl space-y-12 p-6 md:p-8">
        ...
        <section id="executive" className="space-y-6 scroll-mt-20">
          <SectionHeader title="Executive Overview" subtitle="Morning glance — daily KPIs and goal progress" />
          ...
          <Grid numItemsSm={2} numItemsLg={6} className="gap-4">
            ...
            <KpiCard label="Won deals $" value={m.wonDealsTotal ? formatCurrency(m.wonDealsTotal) : "No data"} />
          ...
          <Card>
            <div className="flex items-end justify-between">
              ...
              <Text>Target {formatCurrency(MONTHLY_REVENUE_TARGET)} ({monthProgressPct.toFixed(1)}%)</Text>
            </div>
        ...
        <section id="revenue-mix" className="space-y-6 scroll-mt-20">
          <SectionHeader title="Revenue Mix" subtitle="Brief §3 buckets vs $2M monthly target mix" />
          ...
          <Grid numItemsLg={2} className="gap-6">
            <Card>
              ...
              <BarChart
                className="mt-4 h-72"
                data={m.byRevenueBucket.map((b) => ({ bucket: b.bucket, "Estimate Low": b.estLow }))}
                index="bucket"
        ...
        <section id="lead-quality" className="space-y-6 scroll-mt-20">
          <SectionHeader title="Lead Quality + Source" subtitle="Where the best leads are coming from" />
          ...
          <Grid numItemsLg={2} className="gap-6">
            <Card>
              ...
              <BarList
                className="mt-4"
                data={m.byService.slice(0, 10).map((s) => ({ name: s.service, value: s.leads }))}
                valueFormatter={formatNumber}
              />
            </Card>
            <Card>
        ...
        <section id="strategic" className="space-y-6 scroll-mt-20">
          <SectionHeader title="Strategic Pipeline" subtitle="Brief §13 — leads worth chasing first" />
        ...
        <section id="margin" className="space-y-6 scroll-mt-20">
          <SectionHeader title="Margin Protection" subtitle="Leads that need human review before going to customer" />
                  ...
                  <TableBody>
                    {m.reviewFlaggedLeads.slice(0, 25).map((l) => (
                      <TableRow key={l.leadId}>
                        ...
                        <TableCell>
                          <Badge color={l.confidence === "High" ? "emerald" : l.confidence === "Medium" ? "amber" : "rose"}>
                            {l.confidence || "—"}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-right">
        ...
        <section id="operations" className="space-y-6 scroll-mt-20">
          <SectionHeader title="Operations Insights" subtitle="Tuning the engine over time" />
